create PROCEDURE pears."NetQuestionnaire2"(in pWebUserID char(20),in ptaglocation char(3),in pid char(20) default null,in pgroup char(20) default null)
...
begin
  ...
  select tag.sortorder,tag.tagid,0 as sortX,cast(null as smallint),cast(null as char(4)),0 as rectype,isnull(tag.longdescription,tag.description),tag.tagtype,tag.minstep,tag.units,
    ...
    from tag where tag.taglocation = ptaglocation and ((pgroup is null and (if ispublic = 1 then tag.publiconweb else tag.publishtoweb endif) = 1)  or (exists(select * from iqxNetTagGroupMember where iqxNetTagGroupID=pgroup and tagid=tag.tagid and taglocation=tag.taglocation))) union all
  select tag.sortorder,tag.tagid,3-(isnull(tagchoice.subchoice,0)+1) as sortX,tagchoice.sortorder,tagchoice.tagchoiceid,isnull(tagchoice.subchoice,0)+1 as rectype,
    ...
    from tagchoice key join tag where tag.taglocation = ptaglocation and ((pgroup is null and (if ispublic = 1 then tag.publiconweb else tag.publishtoweb endif) = 1)  or (exists(select * from iqxNetTagGroupMember where iqxNetTagGroupID=pgroup and tagid=tag.tagid and taglocation=tag.taglocation))) union all
  select tag.sortorder,tag.tagid,2 as sortX,tagchoice.sortorder,tagchoice.tagchoiceid,3 as rectype,null,null,null,null,null,
    ...
    where tagvalue.id = pid and tag.taglocation = ptaglocation and ((pgroup is null and (if ispublic = 1 then tag.publiconweb else tag.publishtoweb endif) = 1)  or (exists(select * from iqxNetTagGroupMember where iqxNetTagGroupID=pgroup and tagid=tag.tagid and taglocation=tag.taglocation))) order by
